...
def create_certificate(payload: CertificateCreate, db: Session = Depends(get_db)):
    existing = db.query(Certificate).filter(Certificate.certificate_number == payload.certificate_number).first()
    if existing: return certificate_dict(existing)
    x = Certificate(
        certificate_number=payload.certificate_number, instrument_id=payload.instrument_id,
        issue_date=payload.issue_date, expiry_date=payload.expiry_date, status=payload.status,
        inspector=payload.inspector or "Inspector", inspection_id=payload.inspection_id,
    )
    db.add(x); db.commit(); db.refresh(x)
    return certificate_dict(x)
...
def verify_certificate(certificate_number: str, db: Session = Depends(get_db)):
    ...
    if not cert:
        return {"status": "Invalid", "message": "Certificate Not Found", "valid": False}
    instrument = db.query(Instrument).filter(Instrument.instrument_id == cert.instrument_id).first()
    result = {
        "status": "Valid" if cert.status == "ACTIVE" else "Invalid", "valid": cert.status == "ACTIVE",
        "certificate_number": cert.certificate_number, "instrument_id": cert.instrument_id,
        "instrument": instrument_dict(instrument) if instrument else None, "issue_date": cert.issue_date,
        "expiry_date": cert.expiry_date, "inspector": cert.inspector, "blockchain_status": "PENDING",
    }
    return result
